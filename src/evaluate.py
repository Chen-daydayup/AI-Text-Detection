...
def visualize_all_tsne():
    ...
    from src.fusion_bert_ppl import (
        BertFusionModel,
        get_ppl_features,
        FUSION_MODEL_PATH,
        PPL_SCALER_PATH,
    ...
    if not os.path.exists(FUSION_MODEL_PATH):
        raise FileNotFoundError(
            f"未找到训练后的 Fusion 模型权重：{FUSION_MODEL_PATH}\n"
            "请先运行 run_bert_ppl_fusion()，或者直接运行 python main.py。"
        )

    if not os.path.exists(PPL_SCALER_PATH):
        raise FileNotFoundError(
            f"未找到 PPL 标准化器：{PPL_SCALER_PATH}\n"
            "请先运行 run_bert_ppl_fusion()，或者直接运行 python main.py。"
        )

    fusion_model = BertFusionModel().to(device)

    fusion_model.load_state_dict(
        torch.load(FUSION_MODEL_PATH, map_location=device)
    )

    fusion_model.eval()

    with open(PPL_SCALER_PATH, "rb") as f:
        ppl_scaler = pickle.load(f)

    ppls, _ = get_ppl_features(texts, scaler=ppl_scaler)

    fusion_tokenizer = BertTokenizer.from_pretrained(BERT_MODEL_NAME)

    emb_fusion = []

    with torch.no_grad():
        for txt, ppl in zip(texts, ppls):
            toks = fusion_tokenizer(
                txt,
                return_tensors="pt",
                truncation=True,
                max_length=256,
...
def save_error_examples(model_name, y_true, y_pred, texts):
    ...
    with open(f"results/error_samples_{model_name}.txt", "w", encoding="utf-8") as f:
        ...
        f.write("False Positive (真Human→预AI)\n\n")
