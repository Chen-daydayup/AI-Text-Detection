...
| BERT Fine-tune     | 0.9263       | 0.9798           |
| BERT+PPL Fusion    | 0.8518       | 0.9299           |

基于上述实验结果，得出以下结论：
1. 预训练语言模型（BERT Fine-tune）在该任务中表现最佳，准确率达到0.9263，AUC达到0.9798，显著优于传统机器学习方法。
2. 传统词法特征（TF-IDF）相比简单统计特征（Length、Perplexity）具有更强的判别能力，验证了词汇分布信息在人机文本区分中的作用。
3. 在特征融合实验中，BERT语义特征与困惑度特征的Early Fusion未能进一步提升性能，主要原因有三点：一是融合结构中BERT仅用于固定特征提取，未针对任务微调；二是分类器采用逻辑回归，表达能力弱于BERT微调的非线性分类头。三是BERT的强语义表示已包含困惑度相关信息，简单拼接会造成特征冗余。
